...
async def link_command(inter: disnake.AppCmdInter, ign: str=commands.param(description="Your IGN", min_length=1, max_length=16)):
...
async def moderation_give_role_command(inter: disnake.AppCmdInter, member: disnake.Member, role: disnake.Role):
    if not await misc.validate_mod_cmd(inter, role): return
...
async def moderation_ban_command(inter: disnake.AppCmdInter, member: disnake.Member, reason: str):
    if not await misc.validate_mod_cmd(inter, member.top_role): return
...
async def moderation_unban_command(inter: disnake.AppCmdInter, player: str = commands.Param(
    ...
    if not await misc.validate_mod_cmd(inter): return
...
async def moderation_force_update_command(inter: disnake.AppCmdInter, member: disnake.Member, ign: str):
    if not await misc.validate_mod_cmd(inter): return
...
async def update_players_task_single(uuid: str, member: disnake.Member, session: Optional[aiohttp.ClientSession]=None):
...
async def update_linked_players_task():
    async with aiohttp.ClientSession() as session:
        ...
        for uuid, discord_id in linked_users.items():
            if discord_id not in member_dict: continue
            await update_players_task_single(uuid, member_dict[discord_id], session=session)
            await asyncio.sleep(2) # rate limit sucks ass
...
async def ensure_all_verified_task():
    ...
    for verified_role in [await misc.get_role(bot, role_id) for role_id in config.REQUIRES_VERIFICATION]:
        if verified_role is None: continue
